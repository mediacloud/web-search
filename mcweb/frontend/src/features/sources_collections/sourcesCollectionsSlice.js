...
const sourcesCollectionsSlice = createSlice({
  ...
  reducers: {
    ...
    setCollectionSourcesAssociations: (state, { payload }) => payload.sources.map((source) => (
    ...
    dropSourceCollectionAssociation: (state, { payload }) => (
      state.filter((assoc) => (assoc.source_id !== Number(payload.data.source_id)
        || assoc.collection_id !== Number(payload.data.collection_id)))),
  },
});
...
} = sourcesCollectionsSlice.actions;
