...
class Scraper:
    ...
    def _add_source_line(self, summary: bool, line: str):
        """
        loging everything to leave trail in log file
        DON'T want newline in logged message!
        summary: True for messages to include even if detail is off.
        """
        if line and line[-1] == "\n":
            line = line[:-1]
        logger.info(">>%s", line) #  drop reeces pieces
        if summary or self.detail:
    ...
    def _scrape_source3(self, source_id: int, homepage: str, name: str):
        ...
        try:
            ...
            self._add_source_line(True, f"  fatal error for rss: {e!r}")
            logger.warning("generate_feed_urls(%s): %r", homepage, e)
        except TimeoutError:
            self._add_source_line(True, "  timeout for rss")
        ...
        try:
            ...
            self._add_source_line(True, f"  fatal error for {GNEWS} discovery: {e!r:.1024}")
    ...
    def _process_urls(self, source_id: int, homepage: str, from_: str, urls: Iterable[str]):
        ...
        for nurl, url in nurls.items():
            ...
            if nurl in self.new_urls:
                self._add_source_line(False, f"  duplicate {from_} feed {url}")
                self._feed_counts.duplicate += 1
            elif nurl in self.old_urls:
                self._add_source_line(False, f"  confirmed {from_} feed {url}")
                ...
                try:
                    ...
                    self._add_source_line(False, f"  added {from_} feed {url}")
                    ...
                    self._add_source_line(False, f"  {from_} feed {url} exists in {ofeed_src}")
    ...
    def scrape_source(self, source_id: int, homepage: str, name: str, extra: str = "") -> ScrapeSourceResult:
        ...
        self._add_source_line(False, f"Scraped source {source_id} ({name}){extra}")
        ...
        self._add_source_line(False, "  " + summary)
