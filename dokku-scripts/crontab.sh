...
# The manage "periodic" command takes one argument, which it expects
# to be the name of a subdirectory of the "periodic" directory, which
# contains numbered scripts (lower numbers run first).  The periodic
# comand in the supervisord container rather than firing up a new
# container just for the duration of the script.  The manage
# "periodic" command DEPENDS ON THIS (it creates .lock files in the
# same directory that the script was found in)!!!
DOKKU_RUN_PERIODIC="/usr/bin/dokku enter $APP supervisord python mcweb/manage.py periodic"
...
# be accessing Media Cloud somewhere in the world.  These are just
# initial choices, and might need to be adjusted!!!
...
# minutes semi-randomly chosen to avoid 60/30/15/10 minute multiples
#
# MIT Dorm "Random House" says 17 is the most random number
# Add scripts to be run daily to periodic/daily as nnn.description
17 $HOUR * * * root $DOKKU_RUN_PERIODIC daily > $LOGDIR/$APP-daily.log 2>&1

# 42 is the ultimate answer to life, the universe and everything
# Add scripts to be run daily to periodic/daily as nnn.description
42 $HOUR * * 6 root $DOKKU_RUN_PERIODIC weekly > $LOGDIR/$APP-weekly.log 2>&1
